...
const gameInit = () => {
	for (let i = 0; i < N; i++) {
		...
		box.innerHTML = "<p>" + (i+1).toString() + "</p>"
...
const cfullButton = () => {
	...
	if (isFull[x]) {
		cfull.innerHTML = '<p>' + (isLock[x] ? '解锁' : '') + '取餐</p>';
		cfull.onclick = () => {
			if (isLock[x]) {
